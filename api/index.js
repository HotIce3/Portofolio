...
app.post('/api/admin/projects', authMiddleware, async (req, res) => {
  try {
    const { title, slug, description, content, thumbnail, tech_stack, demo_url, github_url, is_featured, is_published } = req.body;
...
app.put('/api/admin/profile', authMiddleware, async (req, res) => {
  try {
    const { name, title, bio, avatar, resume_url, location, email, phone, social_links } = req.body;
...
app.get('/api/admin/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const unreadMessages = await sql`SELECT COUNT(*) as count FROM contact_messages WHERE is_read = false`;
...
app.get('/api/seed', async (req, res) => {
  try {
    ...
    const existingUser = await sql`SELECT id FROM users WHERE email = 'filbertmathew63@gmail.com'`;
